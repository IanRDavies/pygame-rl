...
class TiledLoader(metaclass=abc.ABCMeta):
    ...
    def load_layers(self):
        ...
        categorized into either "background" or "overlay" if the boolean
        property exists and is enabled. All layers will be added to the "all"
        category.
    ...
    def get_tile_positions(self):
        ...
        for layer in layers:
            ...
            if 'tile' in layer.properties:
                ...
                tid_to_tile_name = {
                    v: k for (k, v) in tile_name_to_tid.items()}
...
class TiledRenderer(TiledLoader):
    ...
    def get_tile_size(self):
        return np.array([self.tiled_map.tilewidth, self.tiled_map.tileheight])

    def get_total_tile_num(self):
        return self.tiled_map.maxgid - 1
    ...
    def get_overlays(self):
        ...
        for layer in overlay_layers:
            ...
            if 'sprite' in layer.properties:
                ...
                raise KeyError('"sprite" property in required for the layer {} '
    ...
    def get_po_screenshot(self, pos, radius):
        ...
        The returned screenshot is always a square with the length of
        "tile size" * (2 * radius + 1). The image of the agent is always
        centered. The default background is black is the cropped image is near
        the boundaries.
